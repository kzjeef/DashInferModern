...
class DeepSeek_v3(Model):
    ...
    def __init__(self, torch_model, data_type, derive_type, **kwargs):
        ...
        self.routing_biases = {}
        for name, tensor in torch_model.items():
            if "e_score_correction_bias" not in name:
                continue
            match = re.search(r"layers\.(\d+)", name)
            if match:
                self.routing_biases[int(match.group(1))] = tensor.float().cpu()
    ...
    def _build_graph(self, torch_cfg, derive_type):
            ...
            for key in shared_expert_name_map:
                ...
                if real_name in self.name_adapter.weight_name_segments:
                    self.weight_name_map["decoder.layer.{}.{}".format(
                        i, key)] = self.name_adapter.fullname(
                            real_name).format(i)
            # Softmax fallback keeps the legacy GEMM-bias interpretation.
            # Sigmoid grouped routing passes this tensor to the MoE operator,
            # which applies it after sigmoid as required by DeepSeek-V3.
            if (self.has_correction_bias and
                    torch_cfg.get('scoring_func', 'sigmoid') != 'sigmoid'):
                self.weight_name_map[
        ...
        for i in range(cfg.dec_layer):
            ...
            if i < first_k_dense_replace:
                ...
                gate_uses_routing_bias = (
                    routing_mode == 1 and self.has_correction_bias)
                mlp_gate = self.make_gemm_op(
                    prefix + "mlp.gate", ffn_input,
                    {"with_bias": (self.has_correction_bias and
                                   not gate_uses_routing_bias)})()
                moe_attr = {
                ...
                if gate_uses_routing_bias and i in self.routing_biases:
                    moe_attr["routing_bias"] = self.routing_biases[i]
                mlp_moe = MOE(
                    prefix + "mlp.experts",
