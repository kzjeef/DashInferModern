...
"""Run the dense-only miniature DeepSeek-V3 NVFP4 model on one SM100 GPU."""
...
def main(model_dir, max_length):
    ...
    model_name = "tiny_deepseek_v3_nvfp4"
    ...
    if engine.install_model(runtime_config) != AsStatus.ALLSPARK_SUCCESS:
        raise RuntimeError("failed to install tiny DeepSeek-V3 NVFP4")
    if engine.start_model(model_name) != AsStatus.ALLSPARK_SUCCESS:
        raise RuntimeError("failed to start tiny DeepSeek-V3 NVFP4")
    ...
    print("PASS: tiny DeepSeek-V3 NVFP4 prefill/decode", generated_ids)
